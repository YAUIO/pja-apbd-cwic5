...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnDistinctJobTitles()
    {
        ...
        var jobs = emps.Distinct().Select(e => e.Job); 
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.SelectMany(e => depts.Where(d => d.DeptNo == e.DeptNo)
            .SelectMany(d => grades.Where(g => g.Losal < e.Sal && g.Hisal > e.Sal)
            .Select(s => new {e.EName, d.DName, s.Grade})));  
