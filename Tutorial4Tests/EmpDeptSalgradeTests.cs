...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = from emp in emps
                group emp by emp.DeptNo into dept
                    select new
                    {
                        DeptNo = dept.Key,
                        Count = dept.Count()
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(e =>
            grades.Where(g => g.Losal < e.Sal && g.Hisal > e.Sal).Select(g => new { e.EName, g.Grade }));
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = from emp in emps
                where emp.Sal > (from e in emps where e.DeptNo == emp.DeptNo group e by e.DeptNo into deps select deps.Average(e => e.Sal)).First()
                    select emp.EName; 
